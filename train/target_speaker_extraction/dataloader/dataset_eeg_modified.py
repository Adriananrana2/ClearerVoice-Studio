...
class dataset_eeg(data.Dataset):
    def __init__(self, args, partition):
        ...
        for subject in range(1, 17):
            for trial in range(1, 9):
                eeg_path = f'{self.eeg_direc}{subject}Tra{trial}.npy'
                #eeg_data = np.load(eeg_path)
                mat = loadmat(eeg_path, simplify_cells=True)
                trial = mat['preproc_trials'][0]
                eeg_data = trial['RawData']['EegData'].astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        return np.asarray(mix_audios, dtype=np.float32), np.asarray(tgt_audios, dtype=np.float32), np.asarray(tgt_eegs,
                                                                                                              dtype=np.float32)
